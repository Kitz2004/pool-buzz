  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"72px 24px", gap:"16px" }}>
      <div style={{ display:"flex", gap:"6px" }}>
        {[0,1,2].map(i => (
          <span key={i} style={{ width:8, height:8, borderRadius:"50%", background:"#3DDC84", display:"inline-block", animation:`dot-bounce 1.2s ease-in-out ${i*0.16}s infinite` }}/>
        ))}
      </div>
...
function StreakChip({ streak }) {
  ...
  return (
    <span style={{
      ...
      background: streak > 0 ? "rgba(61,220,132,0.10)" : streak < 0 ? "rgba(255,90,90,0.10)" : "transparent",
      color,
    }}>
      {label}
    </span>
      ...
      {[
        ...
        return (
          <button key={key} onClick={() => onChange(key)} style={{
            ...
            boxShadow:  active ? "0 2px 10px rgba(61,220,132,0.2)" : "none",
          }}>
            {label}
          </button>
        );
...
// ─── MOBILE CARD — Pool ───────────────────────────────────────────────────────
function PoolCard({ player, idx, period }) {
  const isMedal = idx < 3;
  ...
  return (
    <div className="lb-card" style={{
      ...
      <div style={{
        ...
        fontSize: isMedal ? "20px" : "13px", fontWeight:"800",
        color: isMedal ? MEDAL[idx].text : "#555E6B",
      }}>
        {isMedal ? MEDAL[idx].label : idx + 1}
      </div>
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ fontSize:"15px", fontWeight: isMedal?"700":"600", color: isMedal?MEDAL[idx].text:"#E8ECF0", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", marginBottom:"4px" }}>
          {player.name}
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:"8px", flexWrap:"wrap" }}>
          <span style={{ fontSize:"11px", color:"#555E6B" }}>
            <span style={{ color:"#3DDC84", fontWeight:"700" }}>{wins}W</span>
            {" "}<span style={{ color:"#FF5A5A", fontWeight:"700" }}>{losses}L</span>
          </span>
      ...
      <div style={{ flexShrink:0, textAlign:"right" }}>
        {period === "all" ? (
          <>
            <div style={{ fontSize:"18px", fontWeight:"800", color: isMedal?MEDAL[idx].text:"#3DDC84", fontVariantNumeric:"tabular-nums" }}>
              {player.elo_rating ?? "—"}
            </div>
            <div style={{ fontSize:"9px", color:"#555E6B", letterSpacing:"0.1em", textTransform:"uppercase", marginTop:"1px" }}>ELO</div>
          </>
        ) : (
          <>
            <div style={{ fontSize:"18px", fontWeight:"800", color: isMedal?MEDAL[idx].text:"#3DDC84", fontVariantNumeric:"tabular-nums" }}>
            ...
            <div style={{ fontSize:"9px", color:"#555E6B", letterSpacing:"0.1em", textTransform:"uppercase", marginTop:"1px" }}>WINS</div>
          </>
        )}
      </div>
    </div>
...
function SnookerCard({ row, idx }) {
  ...
  return (
    <div className="lb-card" style={{
      ...
      <div style={{ flexShrink:0, width:"34px", height:"34px", borderRadius:"8px", display:"flex", alignItems:"center", justifyContent:"center", background: isMedal?"transparent":"rgba(255,255,255,0.05)", fontSize: isMedal?"20px":"13px", fontWeight:"800", color: isMedal?MEDAL[idx].text:"#555E6B" }}>
        {isMedal ? MEDAL[idx].label : idx + 1}
      </div>
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ fontSize:"15px", fontWeight: isMedal?"700":"600", color: isMedal?MEDAL[idx].text:"#E8ECF0", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", marginBottom:"4px" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"8px", flexWrap:"wrap" }}>
          <span style={{ fontSize:"11px", color:"#555E6B" }}>
            <span style={{ color:"#3DDC84", fontWeight:"700" }}>{row.wins}W</span>{" / "}{row.played} played
          </span>
          <span style={{ fontSize:"11px", color: winPctColor(pct), fontWeight:"600" }}>{pct}</span>
          {row.highest_break != null && <span style={{ fontSize:"11px", color:"#FFC53D", fontWeight:"700" }}>🎱 {row.highest_break}</span>}
        </div>
      </div>
    </div>
  );
...
// ─── DESKTOP TABLE — Pool ─────────────────────────────────────────────────────
function PoolTable({ players, period }) {
  const cols = period === "all"
    ? ["#","Player","ELO","W","L","Win %","Streak"]
    : ["#","Player","W","L","Win %"];

  return (
    <div style={{ borderRadius:"12px", border:"1px solid rgba(255,255,255,0.07)", overflow:"hidden", background:"rgba(255,255,255,0.02)" }}>
      <table style={{ width:"100%", borderCollapse:"collapse" }}>
        <thead style={{ background:"rgba(255,255,255,0.03)", borderBottom:"1px solid rgba(255,255,255,0.07)" }}>
          <tr>
            {cols.map((h, i) => (
              <th key={h} style={{ padding:"12px 16px", fontSize:"10px", fontWeight:"700", letterSpacing:"0.14em", textTransform:"uppercase", color:"#555E6B", textAlign: i <= 1 ? "left" : "right", whiteSpace:"nowrap", width: i===0?"52px":undefined }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {players.map((player, idx) => {
            ...
            const isMedal = idx < 3;
            return (
              <tr key={player.id} style={{ background: isMedal?MEDAL[idx].bg:idx%2===0?"transparent":"rgba(255,255,255,0.012)", borderLeft:`2px solid ${isMedal?MEDAL[idx].border:"transparent"}`, borderBottom:"1px solid rgba(255,255,255,0.04)", transition:"background 0.15s ease" }}>
                <td style={{ padding:"14px 16px", textAlign:"center", verticalAlign:"middle" }}>
                  <span style={{ display:"inline-flex", alignItems:"center", justifyContent:"center", width:"28px", height:"28px", borderRadius:"6px", fontSize:isMedal?"16px":"12px", fontWeight:"700", background:isMedal?"transparent":"rgba(255,255,255,0.05)", color:isMedal?MEDAL[idx].text:"#555E6B" }}>
                    {isMedal ? MEDAL[idx].label : idx+1}
                  </span>
                </td>
                <td style={{ padding:"14px 16px", fontSize:"13px", verticalAlign:"middle" }}>
                  <span style={{ fontWeight:isMedal?"700":"500", color:isMedal?MEDAL[idx].text:"#E8ECF0", fontSize:"14px" }}>{player.name}</span>
                </td>
                {period === "all" && (
                  <td style={{ padding:"14px 16px", textAlign:"right", verticalAlign:"middle" }}>
                    <span style={{ fontWeight:"700", fontSize:"14px", color:isMedal?MEDAL[idx].text:"#3DDC84", fontVariantNumeric:"tabular-nums" }}>{player.elo_rating ?? "—"}</span>
                  </td>
                )}
      ...
      <table style={{ width:"100%", borderCollapse:"collapse" }}>
        ...
        <tbody>
          {rows.map((row, idx) => {
            ...
            const isMedal = idx < 3;
            return (
              <tr key={row.player_id} style={{ background:isMedal?MEDAL[idx].bg:idx%2===0?"transparent":"rgba(255,255,255,0.012)", borderLeft:`2px solid ${isMedal?MEDAL[idx].border:"transparent"}`, borderBottom:"1px solid rgba(255,255,255,0.04)", transition:"background 0.15s ease" }}>
                <td style={{ padding:"14px 16px", textAlign:"center", verticalAlign:"middle" }}>
                  <span style={{ display:"inline-flex", alignItems:"center", justifyContent:"center", width:"28px", height:"28px", borderRadius:"6px", fontSize:isMedal?"16px":"12px", fontWeight:"700", background:isMedal?"transparent":"rgba(255,255,255,0.05)", color:isMedal?MEDAL[idx].text:"#555E6B" }}>
                    {isMedal ? MEDAL[idx].label : idx+1}
                  </span>
                ...
                <td style={{ padding:"14px 16px", fontSize:"13px", verticalAlign:"middle" }}>
                  <span style={{ fontWeight:isMedal?"700":"500", color:isMedal?MEDAL[idx].text:"#E8ECF0", fontSize:"14px" }}>{row.name}</span>
                </td>
...
// ─── POOL PANEL ───────────────────────────────────────────────────────────────
function PoolLeaderboard({ players, loading, error, period }) {
  if (loading) return <LoadingDots />;
  if (error)   return <div style={{ textAlign:"center", padding:"60px 24px", color:"#FF5A5A", fontSize:"14px" }}>⚠ {error}</div>;
  if (players.length === 0) return <div style={{ textAlign:"center", padding:"60px 24px", color:"#555E6B", fontSize:"14px" }}>No players yet. Record a match to get started.</div>;

  // For "This Month", sort by month wins desc, then win% desc
  const sorted = period === "month"
    ? [...players].sort((a, b) => {
        const aw = a.month_wins ?? 0, bw = b.month_wins ?? 0;
        if (bw !== aw) return bw - aw;
        const at = (a.month_wins ?? 0) + (a.month_losses ?? 0);
        const bt = (b.month_wins ?? 0) + (b.month_losses ?? 0);
        const ap = at ? aw / at : 0, bp = bt ? bw / bt : 0;
        return bp - ap;
      })
    : players; // already sorted by elo_rating desc from fetch

  return (
    <>
      <div className="lb-cards">
        <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
          {sorted.map((p, i) => <PoolCard key={p.id} player={p} idx={i} period={period} />)}
        </div>
      </div>
      <div className="lb-table">
        <PoolTable players={sorted} period={period} />
      </div>
    </>
  );
}
...
function SnookerLeaderboard({ rows, loading, error }) {
  ...
  return (
    <>
      <div className="lb-cards"><div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>{rows.map((r, i) => <SnookerCard key={r.player_id} row={r} idx={i} />)}</div></div>
      <div className="lb-table"><SnookerTable rows={rows} /></div>
    </>
  );
}
...
export default function Leaderboard() {
  ...
  // ── Fetch pool: gets all-time stats + this-month stats in one go ──────────
  const fetchPool = useCallback(async () => {
    try {
      // Base player data (all-time ELO + streak)
      const { data: playerData, error: pErr } = await supabase
        .from("players")
        .select("id, name, elo_rating, total_wins, total_losses, total_matches, current_streak")
        .eq("group_id", groupId)
        .order("elo_rating", { ascending: false });
      if (pErr) throw pErr;

      // This month's pool match_players
      ...
      // Aggregate this month per player
      const monthMap = {};
      for (const row of monthData || []) {
        const pid = row.player_id;
        if (!monthMap[pid]) monthMap[pid] = { wins: 0, losses: 0 };
        if (row.is_winner) monthMap[pid].wins++;
        else               monthMap[pid].losses++;
      }
  ...
  return (
    <div style={{
      ...
      {/* Nav */}
      <nav style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 20px", height:"56px", background:"rgba(13,15,20,0.92)", backdropFilter:"blur(12px)", borderBottom:"1px solid rgba(255,255,255,0.07)", position:"sticky", top:0, zIndex:100 }}>
        <a href="/" style={{ display:"flex", alignItems:"center", gap:"10px", textDecoration:"none", flexShrink:0 }}>
          <span style={{ width:"10px", height:"10px", borderRadius:"50%", background:"radial-gradient(circle,#3DDC84 0%,#00A854 100%)", boxShadow:"0 0 8px rgba(61,220,132,0.7)", flexShrink:0 }}/>
          <span style={{ fontSize:"14px", fontWeight:"700", letterSpacing:"0.15em", color:"#E8ECF0", textTransform:"uppercase" }}>Pool Buzz</span>
        </a>
        <div style={{ display:"flex", gap:"4px" }}>
          {[{ href:"/", label:"Record" }, { href:"/leaderboard", label:"Leaderboard" }].map(({ href, label }) => {
            const active = currentPath === href;
            return (
              <a key={href} href={href} className="nav-link" style={{ textDecoration:"none", padding:"7px 14px", borderRadius:"6px", fontSize:"11px", fontWeight:"600", letterSpacing:"0.1em", textTransform:"uppercase", color:active?"#0D0F14":"#8A96A3", background:active?"#3DDC84":"transparent", border:`1px solid ${active?"#3DDC84":"rgba(255,255,255,0.08)"}`, transition:"all 0.15s ease", display:"flex", alignItems:"center", gap:"5px" }}>
                <span>{href==="/"?"🎱":"🏆"}</span>
      ...
      <div className="lb-page" style={{ maxWidth:"900px", margin:"0 auto", padding:"40px 24px 80px" }}>
        ...
        <div style={{ marginBottom:"24px" }}>
          <h1 className="lb-title" style={{ fontSize:"clamp(24px,5vw,40px)", fontWeight:"800", letterSpacing:"-0.02em", lineHeight:1, color:"#E8ECF0", margin:"0 0 12px" }}>
        ...
        {/* Game type tabs */}
        <div className="lb-tabs" style={{ display:"inline-flex", gap:"3px", marginBottom:"16px", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:"10px", padding:"4px" }}>
          {[
            ...
            return (
              <button key={key} onClick={() => setTab(key)} className="lb-tab" style={{ display:"inline-flex", alignItems:"center", justifyContent:"flex-start", gap:"7px", padding:"9px 20px", borderRadius:"7px", border:"none", cursor:"pointer", fontFamily:"inherit", fontSize:"12px", fontWeight:"700", letterSpacing:"0.1em", textTransform:"uppercase", transition:"all 0.18s ease", background:active?"#3DDC84":"transparent", color:active?"#0D0F14":"#555E6B", boxShadow:active?"0 2px 14px rgba(61,220,132,0.22)":"none" }}>
                <span style={{ fontSize:"14px" }}>{icon}</span>
                {label}
                {!cl && count > 0 && (
                  <span style={{ fontSize:"10px", fontWeight:"800", padding:"1px 6px", borderRadius:"20px", background:active?"rgba(0,0,0,0.15)":"rgba(255,255,255,0.06)", color:active?"rgba(0,0,0,0.55)":"#3a4450" }}>{count}</span>
                )}
              </button>
            );
          })}
        ...
        {/* Panels */}
        {tab === "pool"    && <PoolLeaderboard    players={players}  loading={poolLoading}    error={poolError}    period={period} />}
